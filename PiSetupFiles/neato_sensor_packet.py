...
class NeatoSensorPacket(object):
    ...
    def parse_packet(self, packet):
        ...
        neato_outputs = packet.split(chr(26))
        self.response_dict = {resp[:resp.find('\r')]: resp for resp in neato_outputs}
        ranges, _ = self.getScanRanges()
        # convert ranges back to mm for sending as 2 byte ints (shorts)
        ranges = [int(r*1000) for r in ranges]
        sensor_dict['ldsscanranges'] = (len(ranges),
                                        struct.pack('<%sH' % len(ranges),
                                                    *ranges))
        sensor_dict['motors'] = struct.pack('<2d', *self.getMotors())
        sensor_dict['accel'] = struct.pack('<6f', *self.getAccel())
        sensor_dict['digitalsensors'] = struct.pack('<4d', *self.getDigitalSensors())
        self.serialized_packet = pickle.dumps(sensor_dict, protocol=2)
